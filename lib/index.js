...
export default class PhoneInput extends PureComponent {
  constructor(props) {
    super(props);
    let initialCode = props.defaultCode ? undefined : "1";
    let initialNumber = "";
    let initialCountryCode = props.defaultCode ? props.defaultCode : "US";

    // Parse value if it contains country code
    if (props.value && props.value.startsWith("+")) {
      try {
        const parsedNumber = phoneUtil.parse(props.value);
        initialCode = parsedNumber.getCountryCode().toString();
        initialNumber = parsedNumber.getNationalNumber().toString();
        initialCountryCode = phoneUtil.getRegionCodeForNumber(parsedNumber);
      } catch (err) {
        initialNumber = props.value;
      }
    } else {
      initialNumber = props.value || props.defaultValue || "";
    }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      ...
      let newNumber = prevState.number;
      let newCode = prevState.code;
      let newCountryCode = prevState.countryCode;

      if (nextProps.value && nextProps.value.startsWith("+")) {
        try {
          const parsedNumber = phoneUtil.parse(nextProps.value);
          newCode = parsedNumber.getCountryCode().toString();
          newNumber = parsedNumber.getNationalNumber().toString();
          newCountryCode = phoneUtil.getRegionCodeForNumber(parsedNumber);
        } catch (err) {
          newNumber = nextProps.value;
        }
      } else {
        newNumber = nextProps.value || prevState.number;
      }
